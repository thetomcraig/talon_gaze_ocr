---
name: commit-main
description: Commit changes to main, merge main into beta, merge beta into beta-pi, and rebase any feature branch onto beta-pi.
---
...
7. **Switch to `beta-pi`** and pull latest (`git pull`).
8. **Merge `beta` into `beta-pi`** (`git merge beta`). If conflicts arise, ask the user.
9. **If the original branch was a feature branch**:
   a. Switch back to the feature branch.
   b. Rebase onto `beta-pi` (`git rebase beta-pi`). If conflicts arise, ask the user.
10. **Verify no changes were lost**: Compare the affected files on the final original branch with the saved diff from step 1. If any intended hunks are missing — e.g. because they touched code that only exists on the original branch and not on `main` — re-apply those specific changes and amend the feature branch commit.
11. **End on the original branch**.

## Important notes

- Always `git pull` on `main`, `beta`, and `beta-pi` before committing or merging into them.
